...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create bluetouch-05ce
  fly apps create bluetouch-05ce-staging  # ** not mandatory if you don't want a staging environnement **
  ```
...
- Add a `SESSION_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE`,`SUPABASE_ANON_PUBLIC`, `SERVER_URL` and `DATABASE_URL` to your fly app secrets
  ...
  > **Note:** To find your `SERVER_URL`, go to [your fly.io dashboard](https://fly.io/apps/bluetouch-05ce-3a36)
  ...
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bluetouch-05ce-staging
  fly secrets set SUPABASE_URL="https://{YOUR_STAGING_INSTANCE_NAME}.supabase.co" --app bluetouch-05ce-staging
  fly secrets set SUPABASE_SERVICE_ROLE="{STAGING_SUPABASE_SERVICE_ROLE}" --app bluetouch-05ce-staging
  fly secrets set SUPABASE_ANON_PUBLIC="{STAGING_SUPABASE_ANON_PUBLIC}" --app bluetouch-05ce-staging
  fly secrets set DATABASE_URL="postgres://postgres:{STAGING_POSTGRES_PASSWORD}@db.{STAGING_YOUR_INSTANCE_NAME}.supabase.co:5432/postgres" --app bluetouch-05ce-staging
  fly secrets set SERVER_URL="https://{YOUR_STAGING_SERVEUR_URL}" --app bluetouch-05ce-staging
